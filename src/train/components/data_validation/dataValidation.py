...
from src.utils import write_to_json, read_yaml
import pandas as pd
import os, sys
...
class DataValidation:
    ...
    def train_test_drift(self,train_dataframe:pd.DataFrame, test_dataframe:pd.DataFrame)->None:
        try:
           ...
           result = FeatureDrift(n_top_columns=30).\
           run(Dataset(train_dataframe,label=self.target, cat_features=self.categorical_features ),
                Dataset(test_dataframe, label=self.target,cat_features= self.categorical_features))
           ...
           result.save_as_html(self.data_validation_config.drift_report_html_file_path, as_widget=False,connected=True,)
        
           write_to_json(self.data_validation_config.drift_report_yaml_file_path,content=result.value)
        except Exception as e:
            logging.error(BackOrderException(e,sys)) # type: ignore
   


    def initiate_data_validation(self,data_ingestion_artifact:DataIngestionArtifact)->DataValidationArtifact: # type: ignore
        try:
            train_df = pd.read_csv(data_ingestion_artifact.trained_file_path ,index_col=None)
            test_df  = pd.read_csv(data_ingestion_artifact.test_file_path, index_col=None)
